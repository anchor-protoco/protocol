...
export function SupplyInfo({ data }: SupplyInfoProps) {
  const [timeframe, setTimeframe] = useState("6m")
  const percentage = (data.totalSupplied / data.maxSupply) * 100

  const chartData = Array.from({ length: 30 }, (_, i) => ({
    date: `Day ${i + 1}`,
    apr: +(Math.random() * 2 + 0.3).toFixed(2),
  }))

  return (
    <div className="rounded-2xl bg-zinc-900/50 border border-zinc-800 p-6">
      <h2 className="text-xl font-semibold text-white mb-6">Supply Info</h2>
      ...
      <div className="grid grid-cols-2 gap-8 mb-8">
            ...
            <div className="flex items-center justify-center gap-1 text-sm text-gray-400 mb-1">
              ...
              <TooltipProvider delayDuration={200}>
                <InfoTooltip>
                  ...
                  <TooltipContent>
                    <p>Total assets supplied to this market (cash + borrows).</p>
                  </TooltipContent>
                </InfoTooltip>
              </TooltipProvider>
            </div>
            <div className="font-semibold text-white">
              {data.totalSupplied.toFixed(2)} of {data.maxSupply.toLocaleString()}K
            </div>
            <div className="text-sm text-gray-500">
              ${data.totalSuppliedUSD.toFixed(2)} of ${data.maxSupplyUSD.toLocaleString()}K
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
            ...
            <span className="text-sm px-2 py-1 rounded bg-emerald-500/10 text-emerald-400">1×</span>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                ...
                <Tooltip
                  ...
                  itemStyle={{ color: "#22c55e", fontSize: "14px", fontWeight: "600" }}
